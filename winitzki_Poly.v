...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)]  bad: d needs a type parameter. A good term would be d c. However, (b a 5) is a valid mumble. So the next one is valid:
      - [d mumble (b a 5)] good: this is of type "grumble mumble".
      - [d bool (b a 5)] good: this is of type "grumble bool".
      - [e bool true]  good: this is of type "grumble bool".
      - [e mumble (b c 0)] good: this is of type "grumble mumble".
      - [e bool (b c 0)] bad: "e bool" forces the second argument of e to have type bool, not type mumble. A good term would be e mumble c, for example.
      - [c] good: it has non-polymorhpic type "mumble"
*)
...
(** None? We can't write any elements at all, because we don't have any constructor that does not already require to have a baz. But we can use the placeholder symbol "_" and coax Coq into accepting _some_ expression as if it were a valid baz: **)
Check x _ : baz.
Check x (x _) : baz.
Check x (y _ true) : baz.
Check y _ _: baz.

(* we can write infinitely many such expressions, but what do they mean?? 
 We can't really use these expressions much. For instance, this does not work:

Example baz_junk : forall q:bool, y _ q = y _ q.

This is rejected because Coq "can't infer the value of the placeholder". 

But we can prove something trivial like this:
*)
...
Check @combine.
(* No, actually it prints forall X Y : Type, list X -> list  Y -> list (X*Y). *)
...
Proof.
  intros. (*** simpl does not do anything! Why?? "compute" works but "simpl" does not. I wish they explained in more detail what these tactics actually do. *) reflexivity. Qed.
  
Theorem curry_uncurry : forall (X Y Z : Type)
                               (f : (X * Y) -> Z) (p : X * Y),
  prod_uncurry (prod_curry f) p = f p.
Proof.
intros. (* "compute" or "simpl" do nothing useful at this point! *)
destruct p. (* this is the important thing to do! Remember that a pair is just an inductive type with one constructor. So we can just "destruct" it into two values. *)
Case "p=(x,y)".
reflexivity. Qed.
...
Case "x0 :: l". simpl. rewrite IHl. reflexivity. Qed.

 
Theorem map_rev : forall (X Y : Type) (f : X -> Y) (l : list X),
  map f (rev l) = rev (map f l).
Proof.
  intros. induction l. reflexivity. simpl. rewrite map_snoc. rewrite IHl. reflexivity. Qed.
...
(* for example, to count the elements in a list, we can fold on + with Y = nat and any X. This is shown below in the definition of fold_length *)
...
(* Here X=bool; "constfun b" is a function that always returns b. The "override" makes a new function that returns true on 3, otherwise it returns b. But we evaluate this new function on 2, not on 3. So the result is b. *)
...
Proof. intros. induction l. Case "[]".  reflexivity.  
Case "x::l". unfold fold_length. simpl.  rewrite <- IHl. (* we do not need another unfold. Apparently, reflexivity is quite clever! *)
reflexivity. Qed.
(** [] *)
...
Check fold. (* ('a ->'b ->'b) -> list 'a -> 'b -> 'b *)

Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
fold (fun v ls => (f v) :: ls) l []. 

(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)

(*** The reason we now need to use "unfold" seems to be that fold_map is defined not directly as a fixpoint but as a "definition" through "fold", and this is not being directly expanded. Once we get to "fold", the fixpoint is being expanded by "simpl". In contrast, "map" is defined directly as a fixpoint and is being expanded right away by "simpl". On the other hand, "reflexivity" is more clever and does expand everything, including "Definition" and "fixpoint". However, "reflexivity" cannot be used in the beginning because the goal is still far and reflexivity fails to unify the two sides of the equation.  We would ideally need a tactic stronger tha "simpl", that does what "reflexivity" does for simplifying and expanding things, but does not attempt to unify and does not fail when unifying is not yet possible. *)
