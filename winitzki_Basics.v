...
Example test_neg_neg : forall (x:bool), negb( negb x) = x.
Proof. induction x. reflexivity. reflexivity. Qed.  
...
(* will this work? no because C does not yield a value of type bat! *)
(* Inductive bat : Type :=
 | B : bat
 | C : bat -> nat.
*)
...
(* Not true. The function S constructs a new number out of the old one. *)
...
(* "simpl" tactic not needed - reflexivity suffices! *)
...
(* another proof:  induction n. reflexivity. reflexivity.  Qed.
What is the difference, and why did I have to say reflexivity twice?
*)
...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof.
 intros m n o. intros A B. rewrite <- A. rewrite <- B. rewrite <- A. reflexivity. Qed.
(** [] *)
...
Theorem mult_S_1 : forall n m : nat,
  ...
  m * (1 + n) = m * m.
Proof.
  intros n m H. rewrite -> plus_1_l. rewrite <- H. reflexivity. Qed.
(** [] *)
...
Abort.
(* let's try induction? *)
(* No, that's complicated: it will have to use the induction hypothesis, and we would have to prove difficult properties of beq_nat. *)
...
Proof.
  intros f H. intros b. rewrite -> H. rewrite -> H. reflexivity. Qed.

(** Now state and prove a theorem [negation_fn_applied_twice] similar
    to the previous one but where the second hypothesis says that the
    function [f] has the property that [f x = negb x].*)

Theorem identity_fn_neg_applied_twice : 
  forall (f : bool -> bool), 
  (forall (x : bool), f x = negb x) ->
  forall (b : bool), f (f b) = b.
Proof.
  intros f H b. rewrite -> H. rewrite -> H. rewrite -> negb_involutive. reflexivity. Qed.
...
Theorem andb_eq_orb : 
  ...
  b = c.
Proof.
 intros b c. destruct b. rewrite -> andb_true. rewrite -> orb_true. intros H. rewrite <- H. reflexivity.
 rewrite -> andb_false. rewrite -> orb_false. intros H. rewrite <- H. reflexivity.
Qed.
...
Fixpoint nat_of_bin (b:bin) : nat :=
match b with
| Ze => O
| Tw c => plus (nat_of_bin c) (nat_of_bin c)
| Tw1 c => S (plus (nat_of_bin c) (nat_of_bin c))
end.

Example bin_ex1 :  (* 1 + 0b10111 = 0b11000. Binary is written right to left! *)
  bin_incr (Tw1 (Tw1 (Tw1 (Tw (Tw1 Ze))))) = Tw (Tw (Tw (Tw1 (Tw1 Ze)))).
Proof. reflexivity. Qed.
Example bin_un1 : (* 0b101 = 5 *)
nat_of_bin (Tw1 (Tw (Tw1 Ze))) = 5.
Proof. reflexivity. Qed.

Check (Tw1 (Tw1 (Tw1 (Tw (Tw1 Ze))))).
Definition binnumber1: bin := Tw1 (Tw1 (Tw1 (Tw (Tw1 Ze)))).

Example bin_nat_incr_1 : 
  nat_of_bin (bin_incr binnumber1) = S (nat_of_bin binnumber1).
Proof. reflexivity. Qed.
Example bin_nat_incr_2 : 
  nat_of_bin (bin_incr (bin_incr binnumber1)) = S ( S (nat_of_bin binnumber1)).
Proof. reflexivity. Qed.


(*Theorem bin_incr_nat_correct: forall b:bin, nat_of_bin (bin_incr b) = S (nat_of_bin b).
Proof. intros b. induction b.
reflexivity.
reflexivity.
unfold bin_incr.
no, this does not work. The first two clauses work, but for the third clause the solution is much longer.
*)
(** [] *)
...
(*
Fixpoint foo (f:bool) (n:nat) : nat := 
match f with
| false => n
| true => foo false n (* will return n in any case *)
end.
*)
...
(* Function that walks over all points of a triangle *)
(* Fixpoint foo (f:nat) (n:nat) : nat := 
match f with
| O => match n with O => O | S y => foo y y  end
| S x => foo x n
end.
*)
(** [] *)
