...
| nil => l2
(*| h::t => h :: (alternate l2 t)  does not work: cannot guess decreasing argument. *)
| h1::t1 => match l2 with
              | nil => l1
...
Eval compute in (sum [1;2;3] [1;4;1]).
...
(** Write down an interesting theorem about bags involving the
    ...
    stuck! *)
Theorem add_increases_count : forall (s:bag)(v:nat),  
ble_nat (count v s) (count v (add v s)) = true.
Proof.
 intros. simpl. replace (beq_nat v v) with (true).
assert (Ble_nat_incr: forall n:nat, ble_nat n (S n) = true).
Case "prove Ble_nat_incr".
induction n. SCase "n=0". reflexivity.
SCase "S n". simpl. rewrite -> IHn. reflexivity.
rewrite -> Ble_nat_incr. reflexivity.
Case "prove beq_nat v v".
induction v. SCase "v=0". reflexivity.
SCase "S v". simpl. rewrite <- IHv. reflexivity.
Qed. (** [] *)
...
Example Absurdity_2 : forall (a b c : nat),
                        true=false ->  [a;b]=[b;c].
Proof.
intros. (* now we are in a proof with a false hypothesis H and some absurd goal. *)
apply ad_absurdum. (* now the goal is true=false since this was the premise of ad_absurdum *)
rewrite H. (* now the goal is false=false *)
reflexivity.
Qed.
...
(* adding this lemma for convenience *)
...
Proof.
  intros. rewrite app_ass. rewrite app_ass. reflexivity.
Qed.

Theorem snoc_append : forall (l:natlist) (n:nat),
  snoc l n = l ++ [n].
Proof.
  intros. induction l as [| h t]. reflexivity.
Case "n=h::t".
...
Proof.
  intros. induction l1. 
Case "[]". simpl. rewrite app_nil_end. reflexivity.
Case "n::l1". simpl. rewrite IHl1. 
rewrite snoc_append.
rewrite snoc_append.
rewrite app_ass.
reflexivity.
...
Case "n::l1".
(* simpl. rewrite IHl1. reflexivity. -- will fail! Need to simplify for various values of n. *)
destruct n.
 SCase "n=0". simpl. rewrite IHl1. reflexivity.
 SCase "S n". simpl. rewrite IHl1. reflexivity.
Qed.
...
Theorem cons_snoc_1: forall (p q:natlist) (v:nat), p ++ (v::q) = (snoc p v) ++ q.
Proof. intros. rewrite snoc_append. rewrite app_ass.
replace (v::q) with ([v]++q). reflexivity. reflexivity. Qed.
(** [] *)
...
(***** not in the previous problem, but in the previous chapter! *)
...
Lemma count_01: forall (s:bag)(n:nat), count 0 (S n :: s) = count 0 s.
Proof. intros. reflexivity. Qed.
Lemma remove_01: forall (s:bag)(n:nat), remove_one 0 (S n :: s) = S n :: remove_one 0 s.
Proof. intros. reflexivity. Qed.
...
Proof.
 ...
 Case "n :: s". destruct n.
  ...
  SCase "S n". rewrite remove_01. rewrite count_01. rewrite count_01. rewrite IHs. reflexivity.
Qed. 
...
Lemma sum_cons: forall (s t:bag)(n:nat), sum (n::s) t = n :: sum s t.
(* here we don't even need induction! *)
Proof. reflexivity. Qed.  

(*** let's only count zeros, otherwise we will have trouble checking equality of arbitrary natural values. *)

Theorem sum_count: forall (s t: bag), count 0 (sum s t) = (count 0 s) + (count 0 t).
Proof.
intros. induction s. Case "s=[]". reflexivity.
Case "n::s". rewrite sum_cons. 
destruct n.
 SCase "n=0". simpl. rewrite IHs. reflexivity.
 SCase "S n". simpl. rewrite IHs. reflexivity.
Qed.
...
SearchAbout rev.
(* we have rev_involutive: forall l : natlist, rev (rev l) = l.
so the easy way to prove the theorem is to compute rev of rev.
 *)
Theorem rev_eql : forall (l1 l2 : natlist), rev l1 = rev l2 -> l1 = l2.
Proof.
 intros. (* We have hypothesis H and goal "l1=l2". Now we need to transform this goal until we can use H. Unlike informal math proofs, we cannot just say "apply rev to both sides". *)
...
(***** Let's try a more direct informal proof: "apply a function to both sides". *)

Lemma apply_function: forall (T T': Type) (a b:T) (f:T->T'), a=b -> f a = f b.
Proof. intros. rewrite H. reflexivity. Qed.
(* this lemma is okay, but it does not seem to help... *)
(*
Theorem rev_eql' : forall (l1 l2 : natlist), rev l1 = rev l2 -> l1 = l2.
Proof. intros l1 l2.
rewrite apply_function.
Abort. *) (* No, this does not work. It seems that the ordinary mathematical consideration, "apply function to both sides", is not clear enough: it needs to be explicit that the function is invertible. Here we just directly apply the inverse function. *)
...
SearchAbout beq_nat. (* have beq_nat_refl: true= beq_nat n n *)

Theorem beq_natlist_refl : forall l:natlist,
  true = beq_natlist l l.
Proof. intros. induction l.
Case "[]". reflexivity.
Case "n::l". simpl. rewrite <- beq_nat_refl. rewrite <- IHl. reflexivity.
Qed.
(** [] *)
