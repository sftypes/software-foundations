...
Inductive bin : Type :=
  | O : bin
  | T : bin -> bin  (** twice **)
  | Tpo : bin -> bin. (** twice plus one **)

Fixpoint to_unary (b : bin) : nat :=
  match b with
  | O => 0
  | T b' => 2 * (to_unary b')
  | Tpo b' => 2 * (to_unary b') + 1
  end.

Fixpoint incr_bin (a : bin) : bin :=
  match a with
  | O => (Tpo O)
  | (T a') => (Tpo a')
  | (Tpo a') => (T (T a'))
...
Fixpoint add_bin (a b : bin) : bin :=
  match a, b with
  | O, _ => b
  | _, O => a
  | (T a), (T b) => (T (add_bin a b))
  | (T a), (Tpo b) => (incr_bin (T (add_bin a b)))
  | (Tpo a), (T b) => (incr_bin (T (add_bin a b)))
  | (Tpo a), (Tpo b) => (T (incr_bin (add_bin a b)))
  end.
