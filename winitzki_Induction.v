...
Proof.
  ...
  Case "b=true".
   ...
   SCase "c=false". rewrite <- H. reflexivity. (* Why do we rewrite, when H is a false hypothesis? Why cannot we derive anything from false? *)
  Case "b=false". (* now we have a completely false hypothesis. Why do we have to continue dancing around with it? *)
  destruct c.
   SCase "c=true". reflexivity.
...
Proof.
  ...
  Case "S n". rewrite <- plus_n_Sm. simpl. rewrite <- IHn. reflexivity. Qed.
...
(* Induction does a destruct but also adds the induction hypothesis. Destruct only destructurizes, not adding any hypothesis.  *)
...
Theorem plus_swap : forall n m p : nat, 
  n + (m + p) = m + (n + p).
Proof.
  intros n m p. rewrite -> plus_comm. assert(H1: n+p=p+n). rewrite -> plus_comm. reflexivity.
...
(** Now prove commutativity of multiplication.  (You will probably
    ...
    handy. *)
(* Note: the proof becomes difficult if we try proving n*Sm=n*m+n instead of n+n*m... *)
Lemma mult_s_r: forall m n: nat, n*S m = n+n*m.
Proof. intros m n. induction n.
Case "n=0". reflexivity.
Case "S n". simpl.  rewrite -> IHn.
...
Case "S m". simpl. rewrite -> IHm. (* now have n+n*m=n*Sm *)
rewrite <- mult_s_r. reflexivity. Qed.
(** [] *)
...
SCase "prove negb negb = id". destruct b. reflexivity. reflexivity.
rewrite -> H. reflexivity. Qed.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
(* prediction: need to destructure b since it comes first in andb. Actually, also need to destructure c, because can't compute negb c in general. Otherwise would have to prove many lemmas about orb x (negb x)= true etc. *)
...
This is wrong: we don't have any rewriting rules for this yet, only for plus with mult. So we have to use induction in n, at least. *)
Proof.
  intros n m p. induction n.
Case "n=0". reflexivity.
...
(* bin_incr will never produce Tw Ze! *)

Fixpoint nat_of_bin (b:bin) : nat :=
match b with
| Ze => O
| Tw c =>  nat_of_bin c + nat_of_bin c
| Tw1 c => S ( nat_of_bin c  +  nat_of_bin c )
end.
(* The "S" is outside in hopes of making proofs simpler. *)
Theorem bin_incr_sound: forall b:bin, nat_of_bin(bin_incr b) = S(nat_of_bin b).
Proof. intros b. induction b.
Case "b=Ze". reflexivity.
Case "Tw b". reflexivity.
Case "Tw1 b".  simpl. rewrite -> IHb. simpl.  
assert(H: forall x:nat, x + S x = S(x+x)).
  SCase "proof of x+(1+x)=1+(x+x)". intros x. simpl. replace (S x) with (x+1). rewrite -> plus_assoc. rewrite -> plus_comm. reflexivity.
    SSCase "x+1 = S x". rewrite plus_comm. reflexivity.
rewrite -> H. reflexivity. Qed.
...
match n with
  | O => Ze
  | S n => bin_incr (bin_of_nat n)
end.

Example bin_of_nat_test1: bin_of_nat 5 = Tw1 (Tw (Tw1 Ze)).
Proof. reflexivity. Qed.

Theorem nat_of_bin_of_nat_sound: forall n:nat, nat_of_bin(bin_of_nat n)=n.
Proof. intros n. induction n.
Case "n=0". reflexivity.
Case "S n". simpl. rewrite -> bin_incr_sound. rewrite -> IHn. reflexivity. Qed.

(* some binary numbers may contain Tw Ze, which is not syntactically equal to Ze but is semantically equal. So nat_of_bin (Tw Ze) gives 0, and the reverse conversion gives just Ze rather than Tw Ze. *)

Example nat_of_bin_not_normalized: bin_of_nat(nat_of_bin(Tw Ze)) = Ze.
Proof. reflexivity. Qed.

Fixpoint is_bin_zero (b:bin):bool :=
match b with
 | Ze => true
 | Tw c => is_bin_zero c
 | Tw1 c => false
...
match b with
 | Ze => Ze
 | Tw c => match (bin_normalize c) with
   | Ze => Ze
   | Tw x => Tw (Tw x)
   | Tw1 x => Tw (Tw1 x)
   end 
 | Tw1 c => Tw1 (bin_normalize c)
end.
*)
Definition aux_tw_normalize (c:bin)(tw:bin):bin :=
match (is_bin_zero c) with
| true => Ze
| false => Tw tw
end.
...
match b with
 | Ze => Ze
 | Tw c => aux_tw_normalize c (bin_normalize c)
 | Tw1 c => Tw1 (bin_normalize c)
end.

Example normalize_example_1: bin_normalize(Tw (Tw1 (Tw (Tw Ze)))) = Tw (Tw1 Ze).
Proof.  reflexivity. Qed.
Example normalize_example_2: bin_normalize (bin_of_nat 6) = bin_of_nat 6.
Proof.  reflexivity. Qed.

Definition bool_of_nat(x:nat):bool :=
match x with
| O => false
| S _ => true
end.

(* auxiliary function for proving true=false -> n=m *)
...
Lemma zero_is_one: true=false -> 0=1.
Proof. intros H. 
...
Lemma numbers_do_not_exist: true=false-> forall n:nat, S n = 0.
Proof. intros H n.
induction n. Case "n=0".
rewrite <- zero_is_one. reflexivity.
...
Lemma bin_zero_to_nat_zero: forall (b:bin), is_bin_zero(b)=true -> nat_of_bin(b)=0.
Proof. intros b H. induction b.
Case "b=Ze". reflexivity.
Case "Tw b". simpl. rewrite -> IHb. reflexivity.
assert(G: is_bin_zero(Tw b)=is_bin_zero(b)). 
  SCase "proving is_bin_zero(Tw b)=is_bin_zero(b)". reflexivity.
rewrite <- G. rewrite -> H. reflexivity.
Case "Tw1 b". assert(H1: is_bin_zero(Tw1 b)=false). reflexivity.
simpl.
assert(H0: true=false). rewrite <- H. rewrite <- H1. reflexivity.
(* now we have true=false and need to prove some nonsense. *)
rewrite -> numbers_do_not_exist. reflexivity.
rewrite <- H0. reflexivity.
Qed.

Definition twice(n:nat):nat := n+n.
Definition twice1(n:nat):nat := S(n+n). 

Lemma nat_bin_twice: forall b:bin, nat_of_bin(Tw b)= twice (nat_of_bin b).
Proof. intros b. induction b. reflexivity. reflexivity. reflexivity. Qed.

Lemma bin_incr_of_nat: forall n:nat, bin_of_nat(S n) = bin_incr (bin_of_nat n).
Proof. intros. reflexivity. Qed.

(*
Lemma bin_nat_twice: forall n:nat, bin_of_nat(twice n) = bin_normalize (Tw (bin_of_nat n)).
Proof. intros n.
induction n as [|n']. Case "n=0". reflexivity.
Case "S n'". unfold twice.  rewrite <- bin_incr_of_nat.
(* stuck with matching! *)
assert(ATw: forall b c:bin, aux_tw_normalize (bin_incr b) c = Tw c).
intros b c. induction b. reflexivity.
simpl. reflexivity.
simpl.
replace (Tw (bin_incr b)) with (bin_incr (bin_incr b)). 
(* no, doesn't work! *)

(* after > 2 hours of attempts, abandoned this exercise. *)

Theorem bin_of_nat_normalized_sound: forall (b:bin), bin_of_nat(nat_of_bin b) = bin_normalize b.
Proof. intros b. induction b.
Case "b=Ze". reflexivity.
Case "Tw b". rewrite -> nat_bin_twice.
*)
(** [] *)
